...
namespace ArtificialBeings
{
    ...
    {
        // This transpiler prevents artificial units from triggering corpse thoughts. Only organic humanlikes trigger it normally.
        ...
        public class GiveObservedHistoryEvent_Patch
        {
            ...
            {
                return humanlike && !ABF_Utils.IsArtificial(corpse.InnerPawn);
            }
        }
            ...
            public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> insts, ILGenerator generator)
            {
                ...
                foreach (CodeInstruction inst in insts)
                {
                    if (inst.Calls(AccessTools.PropertyGetter(typeof(RaceProperties), nameof(RaceProperties.Humanlike))))
                    {
                        ...
                        yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(ABF_Utils), nameof(ABF_Utils.IsConsideredNonHumanlike), new Type[] { typeof(Pawn) }));
