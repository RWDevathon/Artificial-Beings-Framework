...
namespace ArtificialBeings
{
    public class RitualRoleAssignments_Patch
    {
        ...
        {
            [HarmonyPostfix]
            public static void Listener(Pawn p, RitualRole role, Precept_Ritual ritual, ref string __result)
            {
                ...
                if (ABF_Utils.IsArtificialDrone(p) && p.def.GetModExtension<ABF_ArtificialPawnExtension>()?.allowedDroneRitualParticipation?.Contains(ritual.def) != true)
                {
