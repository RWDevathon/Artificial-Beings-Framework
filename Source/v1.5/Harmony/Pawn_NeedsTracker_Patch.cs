    ...
    public class Pawn_NeedsTracker_Patch
    {
        ...
        public class ShouldHaveNeed_Patch
        {
            ...
            public static void Listener(NeedDef nd, ref bool __result, Pawn ___pawn)
            {
                ...
                if (isArtificialNeed && (pawnExtension.artificialNeeds.NullOrEmpty() || !pawnExtension.artificialNeeds.ContainsKey(nd)) && !___pawn.health.hediffSet.hediffs.Any(hediff => hediff.def.causesNeed == nd))
                {
                    __result = false;
                }
            }
