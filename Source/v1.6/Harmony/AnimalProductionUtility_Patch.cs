            ...
            public static void Listener(ref IEnumerable<StatDrawEntry> __result, ThingDef d)
            {
                ...
                foreach (CompProperties compProperty in d.comps)
                {
                    if (compProperty is CompProperties_ResourceProducerArtificial validProperty)
                    {
                        ...
                        statDrawEntries.Add(new StatDrawEntry(StatCategoryDefOf.AnimalProductivity, "ABF_Stat_Animal_ResourceInterval".Translate(), "PeriodDays".Translate(validProperty.resourceIntervalDays), "ABF_Stat_Animal_ResourceIntervalDesc".Translate(), displayPriority - 20));
